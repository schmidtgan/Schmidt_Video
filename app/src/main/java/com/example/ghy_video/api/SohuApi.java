...
import com.example.ghy_video.model.Album;
...
import com.example.ghy_video.model.sohu.Result;
import com.example.ghy_video.model.sohu.ResultAlbum;
import com.example.ghy_video.utils.OkHttpUtils;
...
public class SohuApi extends BaseSiteApi {
    ...
    String url = getChannelAlbnum(channel,pageNo,pageSize);
    ...
    private void doGetChannelAlbumsByUrl(final String url, final OnGetChannelAlbumListener listener) {
        ...
        OkHttpUtils.excute(url, new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                //3.Album存到AlbumList中
                Result result = AppManager.getGson().fromJson(response.body().string(),Result.class);//将response的body变成String,并映射到ResultAblbum
                AlbumList albuumList=toConvertAlbumList(result);
                if (albuumList!=null){
                    if (albuumList.size()>0&&listener!=null){
                        listener.onGetChannelAlbumSuccess(albuumList);
                    }
                }else {
    ...
    private String getChannelAlbnum(Channel channel, int pageNo, int pageSize) {
        return String.format(API_CHANNEL_ALBUM_FORMAT,toConvertChannelId(channel),pageNo,pageSize);
    }
    //频道传过来的值和厂商提供的值不一样,自定义频道ID和真实频道的转换
